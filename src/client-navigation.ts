/** Native-chat links; this entry mounts no panel or floating overlay. */
...
export function SessionOrigin(props: LinkProps): ReactElement | null {
  ...
  const enabled = available(props.sessions, origin.originSessionId)
  const row = props.sessions.list.getSnapshot().byId?.[origin.originSessionId]
  const title = row?.displayTitle ?? row?.title
  return createElement('div', { className: 'conductor-session-origin', 'aria-label': '会话来源' }, [createElement('style', { key: 'style' }, CSS), createElement('span', { key: 'source' }, '由另一会话发起'), title ? createElement('span', { key: 'title', title }, `· ${title}`) : null,
...
interface ClientContext {
  readonly slots: { inject(key: string, callback: () => (() => void)): unknown; register(options: { name: string; key?: string; id?: string; order?: number }, component: (props: never) => ReactElement | null): () => void }
