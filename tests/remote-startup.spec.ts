...
describe('native pre-step while migration storage is opening',()=>{
  it.each([true,false])('fails closed only for an enabled configured bridge (enabled=%s)',async enabled=>{
    const tables=createInMemoryTables(),effects:(()=>unknown)[]=[]
    ...
    apply({get:(name:keyof typeof services)=>services[name],on:(_event:string,callback:typeof hook)=>{hook=callback;return ()=>{}},
      effect:(callback:()=>unknown)=>{const dispose=callback();if(typeof dispose==='function')effects.push(dispose as ()=>unknown)},
